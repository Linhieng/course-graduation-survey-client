...
import type { Survey, SurveyQuestionContent_SingleSelect, SurveyQuestionType } from "@/types"
...
export const useStoreStat = defineStore('storeStat', () => {
    ...
    function getAnswerItemOptions(questionType: SurveyQuestionType, questionOrder: number) {
        if (questionType === 'single_select') {
            const questionContent = surveyData.value?.
                questions[questionOrder - 1].questionContent as SurveyQuestionContent_SingleSelect
            let count = Array(questionContent.titles.length).fill(0).map(() => {
            ...
            answerData.value.forEach(answerItem => {
                const questionAnswerArr = answerItem.structure_json[questionOrder - 1]
