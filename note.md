...
- [ ] isFetch 有点频繁了，可以考虑使用节流函数，但如何更好的处理呢？因为我的请求都是在 store 中。

## 刷新页面，如何重新获取数据

检测到刷新页面不难，但要在哪些编写逻辑代码呢？搭建是 router 的 guard 中！

## 路由菜单的动态渲染

目前的需求挺简单的，只需要自动获取 route 中的内容就可以了。
具体组件的写法就是普通的递归写法，没什么问题。
更高阶一点，就是通过服务器获取路由菜单。
...
使用真的很简单，直接在路由守卫中的 before 中添加 NProgress.start()，在 after 中添加 NProgress.done()。
